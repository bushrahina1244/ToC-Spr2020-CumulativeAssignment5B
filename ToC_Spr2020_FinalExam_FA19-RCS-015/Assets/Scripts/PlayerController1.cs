...
public class PlayerController1 : MonoBehaviour
{
    ...
    private GameObject childobject1;
    ...
    void OnCollisionEnter(Collision coll)
    {
        ...
        if (coll.gameObject.tag == "Pick Up1") //Change tag
        {
            Destroy(coll.gameObject);

            childobject1 = coll.gameObject.transform.Find("Sphere").gameObject;
            temp = childobject1.GetComponent<TextMesh>().text.ToString();
    ...
    void setPalindromeNo()
    {
        if (allcollisions>= 4)
        {
            winText1.text = "The Number of Palindromes is: " + count;
        }
